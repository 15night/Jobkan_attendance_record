...
export class JobcanOperation extends CommonWebDriverAction {
  ...
  async inputAttendanceRecord(year: number, month: number) {
    ...
    const days = await this.driver.findElements(By.xpath("//div[@id='search-result']/table/tbody/tr"))
  ...
  for(const day of days) {
    ...
    const totalMinutes = Number(soRodoJikan.split(":")[0]) * 60 + Number(soRodoJikan.split(":")[1])
    ...
    const addRow = async () => {
      const tasks = this.driver.findElement(By.xpath("//div[@id='edit-menu-contents']/table/tbody"))
      // Rowを追加
      const addButton = await tasks.findElement(By.xpath("//div[@id='edit-menu-contents']/table/tbody/tr[1]/td[5]/span[@class='btn jbc-btn-outline-primary']"))
      await addButton.click()
      await this.driver.findElement(By.xpath(`//div[@id='edit-menu-contents']/table/tbody/tr[last()]/td[2]//option[text()='${project}']`)).click()
      await this.driver.findElement(By.xpath(`//div[@id='edit-menu-contents']/table/tbody/tr[last()]/td[3]//option[text()='${task}']`)).click()
      return await this.driver.findElement(By.xpath("//div[@id='edit-menu-contents']/table/tbody/tr[last()]"))
    }

    const leftMinutes = (totalMinutes - 60)
      // Rowを追加
      const row = await addRow()
      const inputText = ("" + (Math.floor(leftMinutes/60))).padStart(2, "0") + ":" + ("" + (leftMinutes % 60)).padStart(2, "0")
      const input = row.findElement(By.xpath("td[4]/input"))
      await input.sendKeys(inputText)

    // フォーカスを外さないと保存ボタンが有効化しないので適当な要素をフォーカス
    await this.driver.findElement(By.xpath("//div[@id='edit-menu-contents']/table/tbody//tr[1]")).click()
    // 保存
